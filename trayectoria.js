...
export function coeficientesQuintico(xi, xf, ti, tf) {
  ...
  const dx = xf - xi;
  return [
    xi,                     // a0 = posición inicial
    0,                     // a1 = velocidad inicial = 0
    0,                     // a2 = aceleración inicial = 0
    10 * dx / (T ** 3),   // a3
   -15 * dx / (T ** 4),   // a4
     6 * dx / (T ** 5)   // a5
  ];
...
export function generarTrayectoria(xi, yi, zi, xf, yf, zf, ti = 0, tf = 10, T = 0.01) {
  ...
  for (let ts = ti; ts <= tf + 1e-9; ts = Math.round((ts + T) * 10000) / 10000) {
    ...
    x.push(evalPoli(cx, ts, ti)); // xd(t)
    y.push(evalPoli(cz, ts, ti)); // yd(t)
    z.push(evalPoli(cy, ts, ti)); // zd(t)
  }
  return { t, x, y, z };
}
