...
btnGenerar.addEventListener('click', () => {
  ...
  const qFinal = robot.cinematicaInversa(xf, yf, zf);
  ...
  // Generar trayectorias cartesianas 
  const tray = generarTrayectoria(xi, yi, zi, xf, zf, yf, 0, 10, 0.01);
  ...
  for (let i = 0; i < tray.t.length; i++) {
    const q = robot.cinematicaInversa(tray.x[i], tray.y[i], tray.z[i]);
