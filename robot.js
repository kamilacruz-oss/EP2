export class Robot {
  ...
  /**
   * Cinemática inversa derivada de la última columna de 0T3
   * Derivada de la última columna de 0T3:
   * px = q3·cos(q1)·sin(q2)
   * py = q3·sin(q1)·sin(q2)
   * pz = q3·cos(q2) + l1
   * 
  ...
  cinematicaInversa(px, py, pz) {
    const l1 = 50;
    const q1  = Math.atan2(py, px); // giro de la base
    const dz  = pz - l1;
    const rxy = Math.sqrt(px * px + py * py);   // distancia horizontal
    const q3  = Math.sqrt(rxy * rxy + dz * dz); // extensión del brazo    
    const q2  = Math.atan2(rxy, dz); // ángulo de elevación
    return {
